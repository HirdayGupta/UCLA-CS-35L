...
/* Helpful for nthreads to be global, as all threads can access it. */
int nthreads;

/* Another global, a dda to store the values from each thread's processing */
float dataDump[width][height][3];

/* Another global for a variable that needs to be accessed in both main & threads */
scene_t scene;


void* imageSplitter(void* tnum)
{
    Vec3 camera_pos;
    ...
    int startPoint = (*(int*)tnum) * (width/nthreads);
    int endPoint = (*(int*)tnum) == (nthreads-1) ? width : (*(int*)tnum + 1) * (width/nthreads);
    
    /* for every pixel */
    for( int px=startPoint; px<endPoint; px++ )
    {
	    ...
	    dataDump[px][py][0] = gamma( pixel_color[0] ) * max_color;
	    dataDump[px][py][1] = gamma( pixel_color[1] ) * max_color;
	    dataDump[px][py][2] = gamma( pixel_color[2] ) * max_color;

	    /* enforce caps, replace with real gamma */
	    for( int i=0; i<3; i++)
	      dataDump[px][py][i] = max( min(dataDump[px][py][i], 255), 0);
...
main( int argc, char **argv )
{
    ...
    // Commented out because we are now going to implement nthreads > 1
    //    if( nthreads != 1 )
    //    {
    ...
    /* initialize array of thread ids to pass as args to threads */
    int ids[nthreads];
    ...
    {
      ids[t] = t;
      t_fail = pthread_create(&threads[t], NULL, imageSplitter, &ids[t]);
      if (t_fail)
      {
	fprintf(stderr, "Thread creation failed! Error Code: %d. Exiting...\n",
		t_fail);
	exit(t_fail);
      }
    }

    /* join each of the threads, waiting for each of them to finish */
    for (t = 0; t < nthreads; t++)
    {
      t_fail = pthread_join(threads[t], NULL);
      if (t_fail)
      {
	fprintf(stderr, "Error joining threads! Error: %d. Exiting...\n",
		t_fail);
	exit(t_fail);
      }
    }

    /* print out dataDump to stdout */
    for (int i = 0; i < width; i++)
    {
      for (int j = 0; j < height; j++)
      {
	printf( "%.0f %.0f %.0f\n",
		dataDump[i][j][0], dataDump[i][j][1], dataDump[i][j][2]);
      }
      printf("\n");
    }
